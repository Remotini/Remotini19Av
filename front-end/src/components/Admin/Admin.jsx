...
function Admin() {
    ...
    const handleAddUserAdmin=async(e)=>{
        ...
        axios.post("http://localhost:5001/api/admin/add",
        {firstName:firstName,
        lastName:lastName,
        email:email,
        password:password,
    ...
    useEffect(()=>{
        const fetchUsers=async()=>{
            axios.get('http://localhost:5001/api/user')
                .then((res)=>{
                    setUsers(res.data)
                    setOriginalData(res.data.filter(user=>user.role === 'User' || user.role === 'Chef'))
                    setFilteredData(res.data.filter(user=>user.role === 'User' || user.role === 'Chef'))
                    setLoading(false)
    ...
    const handleUpdateUserAdmin=async(e)=>{
        ...
        try{
            axios.post(`http://localhost:5001/api/profile/updateInfos/${selectedUser}`,  formdata)
            ...
            .then((res) => {
                if (res.data.message){
                    ...
                    setFilteredData(prevUsers => prevUsers.map(user => user._id === updatedUser._id ? updatedUser : user));
                    setFirstName('');
                    setLastName('');
                    setEmail('');
                    setPassword('');
        ...
        const result = await Swal.fire({
          ...
          if (result.isConfirmed) {
            try{
                ...
                setFilteredData(prevUsers => prevUsers.filter(user => user._id !== row._id));
    ...
    <div className="tableContainer">
        {loading ? (<h1>Loading...</h1>):( 
            ...
            <div className="AdminTableHead">
             <div> <input type="text" name="" placeholder={`filter by ${filterColumn}`} id="" value={filterValue}  onChange={handleFilter} /></div>
            <div>
        ...
        <div className="adduser">
            ...
            <form className='adduserform' onSubmit={isEditing ? handleUpdateUserAdmin : handleAddUserAdmin}>
    <input type='text'  placeholder='First Name' value={firstName} onChange={e => setFirstName(e.target.value)} />
    <input type='text' placeholder='Last Name' value={lastName} onChange={e => setLastName(e.target.value)} />
    <input type='email' placeholder='Email' disabled={isEditing} value={email} onChange={e => setEmail(e.target.value)} />
    <input type='password' placeholder='Password' disabled={isEditing} value={password} onChange={e => setPassword(e.target.value)} />
